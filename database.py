...
# --- Social media keyword helpers ---

def get_social_keywords():
    with get_db() as conn:
        return conn.execute(
            "SELECT * FROM social_keywords ORDER BY keyword"
        ).fetchall()


def add_social_keyword(keyword):
    with get_db() as conn:
        conn.execute(
            "INSERT INTO social_keywords (keyword) VALUES (%s) ON CONFLICT DO NOTHING",
            (keyword.strip(),),
        )


def delete_social_keyword(keyword_id):
    with get_db() as conn:
        conn.execute("DELETE FROM social_keywords WHERE id = %s", (keyword_id,))


def toggle_social_keyword(keyword_id):
    with get_db() as conn:
        conn.execute(
            "UPDATE social_keywords SET is_active = 1 - is_active WHERE id = %s",
            (keyword_id,),
        )
...
def add_social_post(platform, post_id, author_name, author_handle,
                    content, url, keyword, likes, shares, published_at):
    """Insert a new post; silently skips duplicates. Returns True if newly inserted."""
    with get_db() as conn:
        cur = conn.execute(
            """INSERT INTO social_posts
               (platform, post_id, author_name, author_handle, content, url,
                keyword, likes, shares, published_at)
...
def get_social_posts(platform=None, keyword=None, limit=60):
    ...
    sql += " ORDER BY published_at DESC NULLS LAST, fetched_at DESC LIMIT %s"
    params.append(limit)
    with get_db() as conn:
        return conn.execute(sql, params).fetchall()
...
def delete_social_post(post_id):
    with get_db() as conn:
        conn.execute("DELETE FROM social_posts WHERE id = %s", (post_id,))


def delete_old_social_posts(days=14):
    with get_db() as conn:
        result = conn.execute(
            "DELETE FROM social_posts WHERE fetched_at < "
            "to_char(NOW() - INTERVAL '1 day' * %s, 'YYYY-MM-DD HH24:MI:SS')",
            (days,),
        )
        return result.rowcount
